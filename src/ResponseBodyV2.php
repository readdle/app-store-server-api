...
final class ResponseBodyV2 implements JsonSerializable
{
     ...
     * period.
     * If the subtype is DOWNGRADE, the user downgraded or cross-graded their subscription. Downgrades take effect at
     * the next renewal.
    ...
    /**
     * A notification type that along with its subtype indicates that the user made a change to the subscription renewal
     * status.
     ...
     * If the subtype is empty, the subscription isn't in a grace period, and you can stop providing the subscription
     * service.
     ...
     * successful billing transaction.
     * If the subtype is PRICE_INCREASE, the subscription expired because the user didn't consent to a price increase
     * that requires user consent.
     */
     ...
     * If the subtype is UPGRADE, the user redeemed an offer to upgrade their active subscription that goes into effect
     * immediately.
     * If the subtype is DOWNGRADE, the user redeemed an offer to downgrade their active subscription that goes into
     * effect at the next renewal date.
     * If the user redeemed an offer for their active subscription, you receive an OFFER_REDEEMED notification type
     * without a subtype.
     */
     ...
     * If the price increase requires user consent, the subtype is PENDING if the user hasn't yet responded to the price
     ...
     * sharing.
     * The App Store sends this notification when a purchaser disabled Family Sharing for a product, the purchaser (or
     * family member) left the family group, or the purchaser asked for and received a refund.
     * Your app also receives a paymentQueue(_:didRevokeEntitlementsForProductIdentifiers:) call. Family Sharing applies
     * to non-consumable in-app purchases and auto-renewable subscriptions.
     */
     ...
     * If the subtype is INITIAL_BUY, the user either purchased or received access through Family Sharing to the
     * subscription for the first time.
     * If the subtype is RESUBSCRIBE, the user resubscribed or received access through Family Sharing to the same
     ...
     * Call that endpoint to test if your server is receiving notifications. You'll receive this notification only at
     * your request.
     */
     ...
     * A notification with this subtype indicates that the user downgraded their subscription. Downgrades take effect at
     * the next renewal.
     */
    ...
    /**
     * Applies to the DID_CHANGE_RENEWAL_PREF notificationType.
     * A notification with this subtype indicates that the user upgraded their subscription. Upgrades take effect
     * immediately.
     */
    ...
    /**
     * Applies to the DID_CHANGE_RENEWAL_STATUS notificationType.
     * A notification with this subtype indicates that the user disabled subscription auto-renewal, or the App Store
     * disabled subscription auto-renewal after the user requested a refund.
     */
    ...
    /**
     * Applies to the EXPIRED notificationType.
     * A notification with this subtype indicates that the subscription expired because the user didn't consent to a
     * price increase.
     */
     ...
     * A notification with this subtype indicates that the subscription failed to renew due to a billing issue; continue
     * to provide access to the subscription during the grace period.
     */
    const SUBTYPE__GRACE_PERIOD = 'GRACE_PERIOD';

    /**
     * Applies to the DID_RENEW notificationType.
     * A notification with this subtype indicates that the expired subscription which previously failed to renew now
